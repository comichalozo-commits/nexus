...
class WebFetchTool : BaseTool() {
    ...
    override val description: String = "Rufe den Inhalt einer URL ab und liefere eine Zusammenfassung."
    override val parameters: ToolParameterSchema = ToolParameterSchema(
        type = "object",
        properties = mapOf(
            "url" to ToolProperty(
                type = "string",
                description = "Die abzurufene URL"
            )
        ),
        required = listOf("url")
    )
    ...
    override suspend fun execute(arguments: String): String {
        val urlStr = getStringParam(arguments, "url")
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                val responseCode = connection.responseCode
                if (responseCode != 200) {
                    return@withContext "Fehler: HTTP $responseCode"
                }
                ...
                textContent.take(3000) + if (textContent.length > 3000) "\n... (gekürzt)" else ""
            } catch (e: Exception) {
                "Fehler beim Abrufen der URL: ${e.message}"
            }
        }
    }
}
